...
//
// Updated 2023-05 by David @matir Tomaschik
// Hacked for gestalt by Ron @iagox86 Bowes
...
func main() {
	// A GlobalScape compressed packet
  // 0x000000e2 = the length of the message
  // 0x0000ff7f = packet id for a compressed packet
  // 0x789c0000 = zlib deflate header
	head := []byte{
		0xe2, 0x00, 0x00, 0x00, 0x7f, 0xff, 0x00, 0x00, 0x78, 0x9c,
	}
	...
	f, _ := os.OpenFile("recursive.zlib", os.O_CREATE|os.O_WRONLY, 0666)
	f.Write(whole)
	f.Close()
}
...
func makeGeneric(zhead, head, ztail, tail, crc []byte) (z, whole []byte) {
	if false {
		head0 := inflate(append(zhead, 0, 0, 0, 0xff, 0xff, 1, 0, 0, 0xff, 0xff))
...
func (e CorruptInputError) Error() string {
	return fmt.Sprintf("flate: corrupt input before offset %d", e)
}
...
func (e *ReadError) Error() string {
	return fmt.Sprintf("flate: read error at offset %d: %d", e.Offset, e.Err.Error())
}
...
func (e *WriteError) Error() string {
	return fmt.Sprintf("flate: write error at offset %d: %d", e.Offset, e.Err.Error())
}
